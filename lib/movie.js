...
const omdbEndpoint = `http://www.omdbapi.com/?apikey=${
...
function asyncGetRottenTomatoesUrlFromMovieObj(movieObj, socket, page) {
  return new Promise(async (resolve, reject) => {
    const randomDelay = Math.round(
      Math.random() * (minSecondsTimeout * 1000) + minSecondsTimeout * 1000
    ); // introduce random delay [minSecondsTimeout, 2 * minSecondsTimeout) seconds because I'm getting throttled by RT
    const message = `Delaying ${Math.round(
      randomDelay / 1000
    )} seconds before getting '${movieObj.title}' on RT.`;
...
function asyncScrapeMovieInfo(movie, socket = null, page) {
  return new Promise((resolve, reject) => {
    mdb('movieInfo', { id: movie.id })
      ...
      .then(formattedMovie => {
        const message = `Adding '${formattedMovie.title}' (score: ${
          formattedMovie.rtScore
        }) to DB.`;
        socket.emit('new_msg', { msg: message });
        console.log(message);

        moviesDb
          .get(formattedMovie.rtScore)
          .push(formattedMovie)
          .write();
        resolve();
      })
